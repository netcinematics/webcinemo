...
console.log('a GEMZ module-loaded')
console.log('TODO modGemz')
// debugger;
if(!nx){nx={}}
// nx.module = {};
...
nx.createSpaceGemz = function(){
    ...
    for(var i=0; i<nx.spaceGemz.length; i++){
        ...
        aEnergem.actionManager = new BABYLON.ActionManager(nx.scene); //EDIT-MESH-CLICK-EVENTS-------------------------------------------
        aEnergem.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnPickTrigger, function (evt) {
        ...
        // if(nx.anmz && nx.anmz.orby){nx.anmz.orby.collisionItems.push(aEnergem);} //collision
        nx.spaceGemzActive.push(aEnergem);
    }
...
debugger;
    ...
    nx.scene.registerBeforeRender(function spaceGemCollisionLoopz() {
        ...
        for(var i=0; i<nx.spaceGemzActive.length;i++){ 
            if(nx.spaceGemzActive[i].intersectsMesh(nx.orbyMesh)){
                ...
                var item = nx.spaceGemzActive[i];
                nx.spaceGemzActive.splice(i,1) //deselect
...
nx.createPipeGemz = function(){
    ...
    for(var i=0; i<nx.pipeGemz.length; i++){
        ...
        // if(nx.anmz && nx.anmz.orby){nx.anmz.orby.collisionItems.push(aEnergem);} //collision
        nx.pipeGemzActive.push(aEnergem);
    }
    ...
    nx.scene.registerBeforeRender(function pipeGemCollisionLoopz() {
        ...
        //TODO remove
        // if(nx.zoneAlarmsOn>0){zoneAlarm();}else if(!nx.zoneAlarmsOn){ nx.scene.unregisterBeforeRender(zoneAlarmLoop); } 
        for(var i=0; i<nx.pipeGemzActive.length;i++){ 
            if(nx.pipeGemzActive[i].intersectsMesh(nx.orbyMesh)){
                ...
                var item = nx.pipeGemzActive[i];
                nx.pipeGemzActive.splice(i,1) //deselect
                //TODO - jettison particle.
                // nx.anm.jettisionParticle({item:item,done:function(){
                    // debugger;
                    item.dispose();
...
// createSpaceGemz(); //todo rename to 
